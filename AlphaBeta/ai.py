        ...
        for y in range(8):
            for x in range(8):
                if self.ValidMove(board, x, y):
                    ...
                    points = self.AlphaBeta(boardTemp, 3, minEvalBoard, maxEvalBoard, True)
    ...
    def AlphaBeta(self,board, depth, alpha, beta, maximizingPlayer):
        ...
        if maximizingPlayer:
            ...
            for y in range(8):
                for x in range(8):
                    if self.ValidMove(board, x, y):
                        (boardTemp, totctr) = self.MakeMove(copy.deepcopy(board), x, y)
                        v = max(v, self.AlphaBeta(boardTemp,  depth - 1, alpha, beta, False))
                        alpha = max(alpha, v)
                        if beta <= alpha:
                            break  # beta cut-off
            return v
            ...
            for y in range(8):
                for x in range(8):
                    if self.ValidMove(board, x, y):
                        (boardTemp, totctr) = self.MakeMove(copy.deepcopy(board), x, y)
                        v = min(v, self.AlphaBeta(boardTemp,  depth - 1, alpha, beta, True))
                        beta = min(beta, v)
                        if beta <= alpha:
                            break  # alpha cut-off
            return v
